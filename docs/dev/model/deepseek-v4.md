...
Pass `"chat_template_kwargs":{"enable_thinking":true}` to use the model's
thinking prompt instead. The current API supports string content with system,
user, developer, assistant, and `latest_reminder` roles; tool calls and
multimodal content are not yet exposed by the PyPTO request schema. Generation
defaults to greedy sampling (`temperature=0`).
